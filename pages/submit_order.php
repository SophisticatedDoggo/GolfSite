...
$clubs_num = isset($_POST['clubs_num']) ? (int)$_POST['clubs_num'] : 0;
$putters_num = isset($_POST['putters_num']) ? (int)$_POST['putters_num'] : 0;
$own_grips = isset($_POST['own_grips']) ? (int)$_POST['own_grips'] : 0;
...
");
$stmt->bind_param("ssssiii", $cust_name, $cust_email, $cust_phone, $cust_notes, $clubs_num, $putters_num, $own_grips);
$stmt->execute();
$stmt->close();
...
if ($own_grips === 0) {
    ...
    for ($i = 1; $i <= $clubs_num; $i++) {
        if (!empty($_POST['club_grip_id_' . $i])) {
    ...
    for ($i = 1; $i <= $putters_num; $i++) {
        if (!empty($_POST['putter_grip_id_' . $i])) {
